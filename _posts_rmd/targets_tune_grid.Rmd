...
While model parameters are learned from the data during training, hyper-parameters control *how* models learn from the data. For example for [xgboost](https://xgboost.readthedocs.io/en/stable/tutorials/model.html) hyperparameters include **$$\gamma$$** a regularization parameter that controls the cost of tree complexity and $$\textbf{max\_depth}$$ which limits how deep each tree can grow.
