...
While model parameters are learned from the data during training, hyper-parameters control *how* models learn from the data. For example, [xgboost](https://xgboost.readthedocs.io/en/stable/tutorials/model.html) hyperparameters include $$\mathbf{\gamma}$$ a regularization parameter that controls the cost of tree complexity and $$\mathbf{max_depth}$$ which limits how deep each tree can grow.
